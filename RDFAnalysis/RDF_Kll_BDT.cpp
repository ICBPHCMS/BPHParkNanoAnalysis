...
The output consists of both KEE and KMuMu variables for data
For MC, the output consists of either KEE (--isEE 1) or KMuMu (--isEE 0) variables

Modified macro at https://github.com/amartelli/BParkNanoAnalysis/blob/master/RDFAnalysis/RDF_MC_Kll_jobs.cpp

g++ -Wall -o RDF_Kll_BDT `root-config --cflags --glibs ` RDF_Kll_BDT.cpp 

./RDF_Kll_BDT --JOBid (-1, 1,2..) --inList input_list.txt --outFile outFileName --isMC (0, 1) --isEE (0, 1) --isResonant (0, 1) --testFile /path/to/input_file.root
*/
...
inputFileList.push_back(base + "BuToKJpsi_ToMuMu_probefilter_SoftQCDnonD_TuneCP5_13TeV-pythia8-evtgen/crab_BuToKJpsi_ToMuMu/191025_125744/0000/BParkNANO_mc_2019Oct25_1.root");
  ...
  std::string isEE_ch = isEE ? "1" : "0";
  std::string isResonant_ch = isResonant ? "1" : "0";
  ...
  if(isMC && isEE){
    n = n0.Define("isEE_All", isEE_ch.c_str())
     .Define("isResonant_All", isResonant_ch.c_str())
